...
final class SortedIterables {
   ...
   */
  public static <E> Collection<Entry<E>> sortedCounts(
      Comparator<? super E> comparator, Iterator<E> elements) {
  ...
  public static <E> Collection<Entry<E>> sortedCounts(
    ...
    if (elements instanceof Multiset) {
      ...
      List<Entry<E>> entries = Lists.newArrayList(multiset.entrySet());
      Collections.sort(
          entries, Ordering.from(comparator).onResultOf(new Function<Entry<E>, E>() {
            @Override
  ...
  static <E> Collection<Entry<E>> singletonEntries(Collection<E> set) {
    return Collections2.transform(set, new Function<E, Entry<E>>() {
      @Override
