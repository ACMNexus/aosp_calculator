...
class Logic {
    ...
    private void clearWithHistory(boolean scroll) {
        ...
        if (MARKER_EVALUATE_ON_RESUME.equals(text)) {
            ...
            evaluateAndShowResult(text, Scroll.NONE);
        } else {
            mResult = "";
            mDisplay.setText(
                    text, scroll ? Scroll.UP : Scroll.NONE);
            mIsError = false;
        }
    }
    ...
    private void clear(boolean scroll) {
        mHistory.enter("");
        mDisplay.setText("", scroll ? Scroll.UP : Scroll.NONE);
        cleared();
    }
    ...
    void onEnter() {
        if (mDeleteMode == DELETE_MODE_CLEAR) {
            ...
            evaluateAndShowResult(getText(), Scroll.UP);
        }
    }
    ...
    void onUp() {
        ...
        if (mHistory.moveToPrevious()) {
            mDisplay.setText(mHistory.getText(), Scroll.DOWN);
        }
    }
    ...
    void onDown() {
        ...
        if (mHistory.moveToNext()) {
            mDisplay.setText(mHistory.getText(), Scroll.UP);
        }
    }
